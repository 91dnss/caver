...
/// A single symbol from `.symtab`.
#[derive(Debug, Clone)]
pub struct SymbolInfo {
    /// Symbol name, resolved from `.strtab`. Empty string for unnamed symbols.
    pub name: String,
    /// Symbol value (virtual address for `STT_FUNC` / `STT_OBJECT`).
    pub vma: u64,
    /// Symbol size in bytes (`st_size`).
    pub size: u64,
    /// Symbol type (`st_info & 0xf`), e.g. `STT_FUNC`, `STT_OBJECT`.
    pub sym_type: u8,
    /// Symbol binding (`st_info >> 4`), e.g. `STB_GLOBAL`, `STB_LOCAL`.
    pub binding: u8,
}
...
impl std::fmt::Display for SymbolInfo {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "{} vma={:#x} size={} type={:#x} binding={:#x}",
            self.name, self.vma, self.size, self.sym_type, self.binding
        )
    }
}
...
///
/// Returns an empty `Vec` if no `.symtab` section exists (stripped binary).
/// The null symbol at index 0 is skipped.
pub(crate) fn list_symbols(elf: &ElfFile) -> Result<Vec<SymbolInfo>> {
    let parsed = ElfFile64::<Endianness>::parse(elf.data.as_slice())?;
    let endian = parsed.endian();
    let elf_header = parsed.elf_header();
    ...
    let symtab_sh = sections.iter().find(|s| s.sh_type(endian) == SHT_SYMTAB);

    let symtab_sh = match symtab_sh {
        Some(s) => s,
        None => return Ok(vec![]),
    };

    let strtab_idx = symtab_sh.sh_link(endian) as usize;
    let strtab_sh = sections.get(strtab_idx).ok_or(CaverError::NotElf64)?;
    let strtab_off = strtab_sh.sh_offset(endian) as usize;
    let strtab_sz = strtab_sh.sh_size(endian) as usize;
    let strtab = &elf.data[strtab_off..strtab_off + strtab_sz];

    let sym_off = symtab_sh.sh_offset(endian) as usize;
    let sym_sz = symtab_sh.sh_size(endian) as usize;
    let sym_data = &elf.data[sym_off..sym_off + sym_sz];
    ...
    sym_data
        ...
        .map(|entry| {
            ...
            let name = read_cstr(strtab, st_name).unwrap_or("").to_owned();
...
/// Returns true if `.symtab` is present.
pub(crate) fn has_symtab(elf: &ElfFile) -> Result<bool> {
    let parsed = ElfFile64::<Endianness>::parse(elf.data.as_slice())?;
    let endian = parsed.endian();
    let elf_header = parsed.elf_header();
    ...
    Ok(sections.iter().any(|s| s.sh_type(endian) == SHT_SYMTAB))
}
